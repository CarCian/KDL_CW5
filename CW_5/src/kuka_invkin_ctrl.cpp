...
KUKA_INVKIN::KUKA_INVKIN() : Traj{0.4, 0.0, 0.7,
0.4, 0.0, 0.7,
0.4, 0.0, 0.7,
0.4, 0.0, 0.7,
0.4, 0.0, 0.7,
...
void KUKA_INVKIN::Traj_Init(){

    cout<<"TRAJECTORY INITIALIZATION "<<endl;
...
void KUKA_INVKIN::ctrl_loop() {
	...
	for(int j = 0 ; j < 837; j++){
		...
		//	Starting from the current position (_p_out) 
		//		command the data with an offset
		F_dest.p.data[0] = Traj[3*j];
		F_dest.p.data[1] = Traj[3*j+1];
		F_dest.p.data[2] = Traj[3*j+2];
